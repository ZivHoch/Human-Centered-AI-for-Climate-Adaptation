function DynaSet() {
  ...
  vis.loadHarcodedDatasetFromJavascriptObject = function (datasetId, versionNumber) {
    ...
    for (var index = 0; index < dataObjectKeys.length; index++) {
      ...
      if (filesRead == dataObjectKeys.length) {
        ...
        // console.log("timeLineDataForObjects", timeLineDataForObjects, Object.keys(timeLineDataForObjects).length);
        var numObjects = sum; // Object.keys(timeLineDataForObjects).length;
    ...
    if (sets.length > 0 || operator == "k-set intersections") {
      ...
      switch (operator) {
        ...
        case "exclusive intersection":
          const namesVis = vis.getBaseSetNames().map(([id, name]) => name);
          intersectionElements = intersectionSetList(sets, timestep);
          const otherSets = minus(namesVis, sets); //const otherSets = minus(vis.getBaseSetNames(), sets);
  ...
  //   console.log("getBaseSetNames", window.sets);
  //   console.log("getBaseSetNames values", Object.values(window.sets));
  //   return Object.values(window.sets).map((set) => set.name);
  // };
  vis.getBaseSetNames = function () {
    if (window.sets && typeof window.sets === "object") {
      return Object.values(window.sets).map((set) => {
        return [set.id, set.name]; // Return tuple (id, name)
      });
    } else {
      console.error("window.sets is not defined or is not an object");
      return [];
    }
  };
  ...
  /* trying adding the second visualization*/
  // Sample data for LineUpJS
  ...
  function getSetElements(setName, timestep) {
    ...
    if (inputRawData[timestepIndex][setHash] !== undefined) {
  ...
  function separateExclusiveElements(inputContext) {
    ...
    for (var i = 0; i < attributeArray.length; i++) {
      ...
      if ("1" in degreeDictionary) {
        if (degreeDictionary["1"].indexOf(parseInt(attributeArray[i])) < 0) degreeDictionary["1"].push(parseInt(attributeArray[i]));
      ...
      if (belongingAttributes.length == 1) {
        ...
        if (temphash in separatedContext) {
          ...
          if (tempsortedattrs.length in degreeDictionary) {
            if (degreeDictionary[tempsortedattrs.length].indexOf(temphash) < 0) degreeDictionary[tempsortedattrs.length].push(temphash);
    ...
    for (var i = 0; i < objectArray.length; i++) {
      if (!(objectArray[i] in allObjectsInfo)) {
        var tempweight = calculateWeight(inputContext.objects[objectArray[i]].weights);
        allObjectsInfo[objectArray[i]] = { name: inputContext.objects[objectArray[i]].name, weight: tempweight };
      }
    }
  ...
  function calculateWeight(weightsArray) {
    var returnweight = 0;
    for (var i = 0; i < weightsArray.length; i++) returnweight += weightsArray[i];
    return returnweight;
  }
    ...
    for (var i = 0; i < inputRawData.length; i++) {
      ...
      for (var j = 0; j < nodesArray.length; j++) {
        ...
        if (nodesArray[j] != "added" && nodesArray[j] != "removed") {
          ...
          inputRawData[i][nodesArray[j]]["exclusive"].forEach((x) => {
            tw += x.weight;
          });
        } else wt = 0;
    ...
    for (var i = timestepIndex + 1; i < inputRawData.length; i++) {
      ...
      for (var j = 0; j < nodesArray.length; j++) {
        if (nodesArray[j] != "added" && nodesArray[j] != "removed") {
          ...
          if (objectsInnodeIdArray.indexOf(object.objectid) > -1) {
            var deg = inputRawData[i][nodesArray[j]].degree;
            if (window.degreesAggregated[deg] == 0) return "T" + i + "N" + nodesArray[j];
  ...
  function getContainedBaseSetsFromSetId(setId) {
    const setNames = [];
    vis.getBaseSetNames().forEach(([baseSetId, setName]) => {
      if (window.hashToBinaryDictionary[baseSetId] === window.hashToBinaryDictionary[setId]) {
  ...
  function computeAggregatedGraphStructure() {
    ...
    for (var nodeid in window.nodes) {
      ...
      if (window.degreesAggregated[tempDegreeOfNode] == 0 || tempDegreeOfNode == undefined) {
        ...
        var tempweight = window.nodes[nodeid]["trueWeight"];
        ...
        if (!(tempNewNodeId in window.aggregatedNodes))
          ...
          window.aggregatedNodes[tempNewNodeId]["trueWeight"] += tempweight;
        }
      }
    }
  ...
  function computeSimilarity(row1, row2) {
    ...
    if (typeof row1 == "string" && row1.includes("Degree")) row1Id = "D" + row1.charAt(6);
    else row1Id = "N" + row1;

    if (typeof row2 == "string" && row2.includes("Degree")) row2Id = "D" + row2.charAt(6);
    else row2Id = "N" + row2;
    ...
    for (var j = 1; j < inputRawData.length; j++) {
      ...
      for (var k = 0; k < window.edges.length; k++) {
        ...
        if (
          (window.edges[k].from.includes(row1Id) && window.edges[k].to.includes(row2Id)) ||
          (window.edges[k].to.includes(row1Id) && window.edges[k].from.includes(row2Id))
        ) {
          window.edges[k].objects.forEach((x) => {
            similarityScore += x.weight;
          });

          similarityScore += window.edges[k].objects.length;
          // console.log("similarityScore", k, similarityScore);
  ...
  function computeGraphLayout() {
    ...
    for (var degree in degreeDictionary) {
      if (window.degreesAggregated[degree] == 1) allConceptHashs.push("Degree" + degree);
      else {
        for (var index in degreeDictionary[degree]) {
          var conceptHash = degreeDictionary[degree][index];
          if (allConceptHashs.indexOf(conceptHash) == -1) allConceptHashs.push(conceptHash);
        }
      }
    }
    // need to change
    if (window.sortBy == "degree") {
      ...
      items.sort(function (a, b) {
        if (a[0] == "added" || a[0] == "removed" || b[0] == "added" || b[0] == "removed") return 1;
        else return b[1] - a[1];
      });
      ...
      for (var deg in degreeDictionary) {
        ...
        for (var i = 0; i < degreeDictionary[deg].length; i++) {
          ...
          if (window.degreesAggregated[deg] == 0) conceptsInstabilityTuple.push([conceptId, instabilityWeight]);
          else {
            degreeWeight += instabilityWeight;
          }
        }
        if (window.degreesAggregated[deg] == 1) conceptsInstabilityTuple.push(["Degree" + deg, degreeWeight]);
      }
      ...
      for (var deg in degreeDictionary) {
        ...
        for (var i = 0; i < degreeDictionary[deg].length; i++) {
          ...
          if (window.degreesAggregated[deg] == 0) conceptsInstabilityTuple.push([conceptId, summedCardinality]);
          else {
            degreeWeight += stabilityWeight;
            degreeSummedCardinality += summedCardinality;
            degreePresenceInTimesteps += presenceInTimesteps / numTimesteps;
        ...
        if (window.degreesAggregated[deg] == 1) conceptsInstabilityTuple.push(["Degree" + deg, degreeSummedCardinality]);
      }
      conceptsInstabilityTuple.sort(function (a, b) {
        return b[1] - a[1];
      });
        ...
        if (window.degreesAggregated[degree] == 1 && degreeDictionary[degree].indexOf(window.selectedSetIdForPriority) >= 0)
          ...
          if (window.degreesAggregated[deg] == 1) {
            ...
            for (var i = 0; i < degreeDictionary[deg].length; i++) {
              ...
              if (window.sethashandNameDict[conceptId].includes(window.sethashandNameDict[window.selectedSetIdForPriority])) rows.push([conceptId, degree]);
              else {
    ...
    for (var concept in verticalPosition) {
      if (!(concept in window.hashToBinaryDictionary)) {
        ...
        if (tempName != undefined) {
          if (tempName.substr(tempName.length - 1) === ",") tempName = tempName.slice(0, -1);
    ...
    if (window.edgeThicknessScale != undefined) {
      ...
      for (var z = 0; z < sortedVerticalPositions.length; z++) {
        if (sortedVerticalPositions[z][0].includes("Degree")) {
          ...
          var ht = 0;
          if (tempMax > window.maxUnaggregatedWeight) ht = edgeThicknessScale(tempMax);
    ...
    for (var deg in degreeDictionary) {
      ...
      for (var previousDegree = 1; previousDegree < deg; previousDegree++) {
        ...
        if (window.degreesAggregated[previousDegree] == 1) verticalPositionOfksetRow += 1;
      ...
      ksetRowGroup
        .append("text")
        .text(function () {
          if (window.degreesAggregated[deg] == 0) return "-";
          else return "+";
        })
        .attr({
          // "x": ksetGroupWidth + 10,
          ...
          degree: deg,
          collapsed: function () {
            if (window.degreesAggregated[deg] == 0) return "false";
            else return "true";
          },
        })
        // left table
        ...
        .append("title")
        .text(function () {
          if (window.degreesAggregated[deg] == 0) return "Collapse " + deg + "-set intersection rows";
          else return "Expand " + deg + "-set intersection rows";
        });
    }
      ...
      rowLegend
        ...
        .on("mouseleave", function () {
          ...
          if (baseSets != null) {
            ...
            for (var k = 0; k < baseSets.length - 1; k++) d3.selectAll(".BaseSet" + baseSets[k]).attr("fill", "white");
          }
        });
    ...
    for (var i = 0; i < inputRawData.length; i++) {
      ...
      for (var set in inputRawData[i]) {
        ...
        for (var j = 1; j <= window.numSets; j++) {
          if (!(j in degreeDistributionPerTimeStep[i])) window.degreeDistributionPerTimeStep[i][j] = { count: 0, objects: [] };
        }
      }
    ...
    for (var i = 0; i < inputRawData.length; i++) {
      ...
      timestepLabel.append("rect").attr({
        ...
        y: function () {
          if (window.currentDatasetId != "image") return topPadding + heightOfDegreeDistributionBox + maxHeightOfNode - bbox.height / 2 - extra;
          else return topPadding + heightOfDegreeDistributionBox + maxHeightOfNode - bbox.height + extra / 2;
        },
        fill: guiParams.highlightColor,
        visibility: "hidden",
        width: bbox.width + 2 * extra,
        height: function () {
          if (window.currentDatasetId != "image") return bbox.height + 2 * extra;
      ...
      var tlabel = timestepLabel
        ...
        .attr({
          ...
          class: "timestepLabel",
          sort: function () {
            if (window.selectedTimestepForSorting == undefined || window.selectedTimestepForSorting != i) return "false";
    ...
    var maxEdgeWeight = -1;
    for (var i = 0; i < edges.length; i++) {
      // console.log("edges ", edges[i].objects);
      var wt_edfes = 0;
      edges[i].objects.forEach((x) => (wt_edfes += x.weight));

      // var wt = edges[i].objects.weight;
      if (minEdgeWeight > wt_edfes) minEdgeWeight = wt_edfes;
      if (maxEdgeWeight < wt_edfes) maxEdgeWeight = wt_edfes;
    }
    ...
    for (var nodeId in window.nodes) {
      var wt = 0;
      // window.nodes[nodeId].forEach((x) => (wt += x.trueWeight));
      // console.log(wt);
      // console.log(window.nodes[nodeId]);
      var wt = window.nodes[nodeId].trueWeight;
      window.nodes[nodeId].weight = wt;
      // console.log("wt", wt);
      if (minNodeWeight > wt) minNodeWeight = wt;
      if (maxNodeWeight < wt) maxNodeWeight = wt;
    }
    if (window.edgeThicknessScale == undefined) {
      // console.log("edgeThicknessScale", maxNodeWeight, maxEdgeWeight);
    ...
    for (var i = 0; i < edges.length; i++) {
      ...
      if (nodesAtAdjacentTimesteps(source, destination)) {
        ...
        if (inedges in window.nodes[destination]) {
          ...
          else if (window.nodes[source].y > window.nodes[destination].y) direction = "comingup";
    ...
    for (
      var i = 0;
      i < edges.length;
      i++ // for(var i=edges.length-1; i>=0; i--)
    ) {
      var source = edges[i].from;
      var destination = edges[i].to;
      ...
      if (nodesAtAdjacentTimesteps(source, destination)) {
        ...
        for (var j = 0; j < window.nodes[source].outedges.length; j++) {
          window.nodes[source].outedges[j].objects.forEach((x) => {
            tempweights += x.weight;
          });
          // tempweights += window.nodes[source].outedges[j].objects[0].weight;
        }
        ...
        if (destination.includes("removed")) {
          if (!("inEdgesForRemovedNode" in window.nodes[destination])) window.nodes[destination]["inEdgesForRemovedNode"] = [];
        ...
        if (source.includes("added")) classOfEdge += "added ";
          ...
          if (nodesAtAdjacentTimesteps(source, destination)) classOfEdge += "consecutive ";
          else classOfEdge += "non-consecutive ";
        } else {
          if (nodesAtAdjacentTimesteps(source, destination)) classOfEdge += "moved ";
        }
        edges[i]["x1"] = x1;
        edges[i]["y1"] = y1;
        edges[i]["x2"] = x2;
        ...
        var edgeScaleValue = 0;
        edges[i].objects.forEach((x) => {
          edgeScaleValue += x.weight;
        });
        nonConsecutiveEdgesOverTimesteps.push([sourceTimestep, destinationTimestep, edgeThicknessScale(edgeScaleValue)]);
        var a;
        ...
        y1 = window.nodes[source].y + edgeThicknessScale(edgeScaleValue) / 2;
        var x1 = window.nodes[source].x + widthOfNode;
      ...
      .on("mouseenter", function (d) {
        ...
        if (setid != null) {
          ...
          var baseSets = d3
            ...
            .attr("baseSets")
            .split(",");
          for (var k = 0; k < baseSets.length; k++) d3.selectAll(".BaseSet" + baseSets[k]).attr("fill", guiParams.highlightColor);
        } else {
    ...
    for (var i = 0; i < window.filenames.length; i++) timeLabelToIndexDictionary[window.filenames[i]] = i;
  ...
  function drawAggregateOption(svg) {
    ...
    var config = {
      ...
      changeHandler: function (option) {
        ...
        // Change handler code goes here
        // document.getElementById("selectedInput").value = option.label;
        if (option.value == "all") for (var i = 1; i < window.degreesAggregated.length; i++) window.degreesAggregated[i] = 1;
    ...
    var config2 = {
      ...
      changeHandler: function (option) {
        ...
        // Change handler code goes here
        // document.getElementById("selectedInput").value = option.label;
        if (option.value == "all") for (var i = 1; i < window.degreesAggregated.length; i++) window.degreesAggregated[i] = 0;
  ...
  function populateAdditionalAttributeArea() {
    ...
    if (datasetSelection.currentLoadedDatasetIndex == 7) {
      ...
      svg.append("path").attr("d", path(geojson)).style("fill", "#eae5e5").style("stroke", "white");
  ...
  function createElementList() {
    ...
    function drawElementListRows(objectArray) {
      ...
      var rows = table
        ...
        .on("click", function (dataitem) {
          ...
          if (!$(this).hasClass("highlighted")) {
            ...
            const elementWeight = allObjectsInfo[d].weight;
            const elementWeightText = elementWeight === 1 ? "simulation" : "simulations";
            if (datasetSelection.currentId === "image") {
              ...
              $("#highlightedElement").html(`
                <div class="simulation-info">
                  <style>
                    ...
                    .element-weight {
                      color: ${guiParams.highlightColor}; /* Green color */
                      font-weight: 700;
                    }
                  ...
                  <h1 class="simulation-title">
                    <span class="element-name">${elementName}</span> <br />
                    represents <span class="element-weight">${elementWeight}</span> ${elementWeightText}
                  </h1>
                </div>
              `);
            }
      ...
      rows.each(function (dataitem) {
        var d = dataitem[3];
        var coloredBarTd = d3.select(this).append("td").attr("class", "firstColumn");
  ...
  function aggregatedIntersections(degree, timestep) {
    ...
    for (var conceptId in window.inputRawData[timestepIndex]) {
      ...
      if (degree == concept.degree) {
        ...
        for (var i = 0; i < concept.exclusive.length; i++) tempobjectIdArray.push(concept.exclusive[i].objectid);
  ...
  function drawEdges(edges, group) {
    ...
    edgeGroup
      ...
      .attr("stroke-width", function (d) {
        // console.log("d.objects", d.objects);
        var dWeight = 0;
        d.objects.forEach((x) => {
          dWeight += x.weight;
        });
        return edgeThicknessScale(dWeight);
      })
      .on("mouseenter", function () {
      ...
      .text(function (d) {
        ...
        if (wt == 1) return wt + " simulation";
        else return wt + " simulations";
      });
  }
    ...
    for (var i = 0; i < inputRawData.length - 1; i++) {
      ...
      for (var j = 0; j < nodesArray.length; j++) {
        if (nodesArray[j] != "added" && nodesArray[j] != "removed") {
          ...
          for (var k = 0; k < tempobjects.length; k++) {
            if (objectIdsArray != undefined && objectIdsArray.indexOf(tempobjects[k].objectid) >= 0) objectsInSelectedGroup.push(tempobjects[k]);
          }
          for (var k = 0; k < objectsInSelectedGroup.length; k++) {
            ...
            var deg = inputRawData[i][nodesArray[j]].degree;
            if (window.degreesAggregated[deg] == 1) currentNodeId = "T" + i + "D" + deg;
  ...
  function findY2Weight(destinationNode, direction, edgeid, group) {
    ...
    // {
    // console.error("direction", destinationNode.inedges);
    temparray = destinationNode.inedges.comingdown;
    var tempweight = 0;
    for (var i = 0; i < temparray.length; i++) {
      if (temparray[i].id != edgeid) {
        temparray[i].objects.forEach((x) => {
          tempweight += x.weight;
        });
  ...
  function reorderEdges(edges) {
    ...
    for (var i = 0; i < edges.length; i++) {
      ...
      if (fromNodeHash.slice(3) === toNodeHash.slice(3)) edgeDirectionDictionary[fromNodeHash].same.push(edges[i]);
  ...
  function sortIncomingEdgeBasedOnDestinationYLocation(unsortedEdges, dir) {
    return unsortedEdges.sort(function (a, b) {
      ...
      else if (a.to.includes("removed")) return nodes[a.from].y - nodes[b.from].y;
  ...
  function nodesAreSameSetsInDifferentTimesteps(source, destination) {
    ...
    if (source.substr(3, source.length) === destination.substr(3, destination.length)) {
      if (!(source.substr(1, 1) === destination.substr(1, 1))) returnValue = true;
    }
    return returnValue;
  }
    ...
    for (var i = 0; i < lattices.length; i++) {
      for (var cid in lattices[i].concepts) {
        if (!lattices[i].concepts[cid].isDummy && lattices[i].concepts[cid].name.length > 0) {
          var curSum = 0;
          lattices[i].concepts[cid].objects.forEach((x) => {
            Object.values(x.weights).forEach((w) => {
              curSum += w;
            });
          });
          // console.log(curSum);
          // // console.log(lattices[i].concepts[cid].objects);
          if (curSum > max) {
            max = curSum;
          }
        }
  ...
  function calculateObjectTimelineData(lattice) {
    ...
    for (var i = 0; i < window.numberOftimesteps; i++) versions.push(Math.pow(2, i));
      ...
      for (i = 0; i < concept.children.length; i++) {
        ...
        if (layerDelta == 1) {
          ...
          for (
            ...
            currentLayerIndex++ // for (var currentlayerIndexinLayersArray =0; currentlayerIndexinLayersArray< Object.keys(layers).length; currentlayerIndexinLayersArray++ )
          ) {
            //a quick fix
